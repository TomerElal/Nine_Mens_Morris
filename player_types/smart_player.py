...
class SmartPlayer(Player):
    ...
    def get_action(self, state, type_of_required_action=MoveType.MOVE_PIECE,
        ...
        opponent_move_performed_a_mill = self.check_if_opponent_can_perform_a_mill(state, type_of_required_action)
        print_result = ''
        if (type_of_required_action == MoveType.MOVE_PIECE
            ...
            smart_can_perform_a_mill = self.check_if_smart_player_can_perform_a_mill(state, smart_player_poss_actions)
    ...
    def check_if_opponent_can_perform_a_mill(self, state, type_of_required_action):
        opponent_poss_moves = state.get_opponent_legal_actions(self.get_player_color())
    ...
    def check_if_smart_player_can_perform_a_mill(self, state, smart_player_poss_actions):
        for action in smart_player_poss_actions:
            new_pos = action
            ...
            can_perform_a_mill = utils.utils.move_performed_a_mill(new_pos, state, self.get_player_color())
            if state.player1.num_of_pieces_left_to_place + state.player2.num_of_pieces_left_to_place == 0:
                state.set_cell_state(action[0], self.get_player_color())
