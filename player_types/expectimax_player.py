...
class ExpectimaxPlayer(Player):
    ...
    def get_action(self, state, type_of_required_action=MoveType.MOVE_PIECE):
        ...
        if type_of_required_action == MoveType.MOVE_PIECE:
            print_result = perform_action_to_console(PLAYER_CHOOSE_MOVEMENT, EXPECTIMAX_PLAYER,
                                                     PLAYER_MOVED_PIECE, result)

        elif type_of_required_action == MoveType.PLACE_PIECE:
            print_result = perform_action_to_console(PLAYER_CHOOSE_PLACEMENT, EXPECTIMAX_PLAYER,
                                                     PLAYER_PLACED_PIECE, result)

        else:
            print_result = perform_action_to_console(PLAYER_CHOOSE_REMOVAL, EXPECTIMAX_PLAYER,
